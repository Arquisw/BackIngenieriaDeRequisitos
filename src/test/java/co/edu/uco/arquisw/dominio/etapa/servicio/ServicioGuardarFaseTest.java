...
class ServicioGuardarFaseTest {
    ...
    void guardarExitoso()
    {
        ...
        var servicio = new ServicioGuardarFase(faseRepositorioComando,faseRepositorioConsulta);

        Mockito.when(faseRepositorioConsulta.consultarProyectoPorID(Mockito.anyLong())).thenReturn(ProyectoDto);
    ...
    void deberiaValidarLaExistenciaPreviaDelProyecto() {
        ...
        Mockito.when(faseRepositorioConsulta.consultarProyectoPorID(Mockito.anyLong())).thenReturn(null);
        Mockito.when(faseRepositorioComando.guardar(Mockito.any(Fase.class), Mockito.anyLong())).thenReturn(1L);
